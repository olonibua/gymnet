  ...
  const fetchRandomUsers = async (isInitialLoad = false) => {
    try {
      ...
      while (users.length < 4 && attempts < maxAttempts) {
        ...
        if (randomUser && 
            !users.some(user => user.$id === randomUser.$id)) {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch random users';
...
function FeatureCard({ title, description, icon }: { 
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg sm:text-xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-violet-500 to-fuchsia-500">{title}</h3>
